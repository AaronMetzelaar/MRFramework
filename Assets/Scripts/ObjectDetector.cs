...
public class ObjectDetector : MonoBehaviour
{
    ...
    [Tooltip(
        "The threshold for the shape matching algorithm. The higher the value, the less sensitive the algorithm is to shape differences."
    )]
    [Range(0.0f, 1.0f)]
    public float objectMatchThreshold = 0.2f;
    ...
    [Tooltip(
        "The margin in pixels between two detected objects. Increase this value if objects are detected multiple times or recreated too often."
    )]
    [Range(0.0f, 100.0f)]
    public float positionMargin = 50f;

    [Tooltip(
        "The margin in pixels between contours to be merged. Increase this value if objects are detected as multiple contours."
    )]
    [Range(0.0f, 100.0f)]
    public float contourMergeMargin = 50f;

    [Tooltip(
        "The margin between the hues of the detected object and the initialized object. Increase this value if objects are detected multiple times or recreated too often."
    )]
    [Range(0.0f, 1.0f)]
    ...
    public void Initialize()
    {
        ...
        calibratorData =
            calibrator.CurrentCalibratorData
            ?? throw new Exception("Calibrator data not found. Please calibrate first.");
    ...
    public void StopDetecting()
    {
        ...
        Mat diffImage = objectInitializer.SubtractImages(
            OpenCvSharp.Unity.TextureToMat(webCamTexture),
            calibratorData.BaseImage
        );
    ...
    private void UpdateObjects()
    {
        ...
        using Mat undistortedCroppedImage = calibrator.GetUndistortedCroppedImage(
            image,
            calibratorData.TransformationMatrix,
            calibratorData.CameraMatrix,
            calibratorData.DistortionCoefficients
        );
        using Mat subtractedImage = objectInitializer.SubtractImages(
            undistortedCroppedImage,
            calibratorData.BaseImage
        );
        using Mat transformedImage = objectInitializer.TransformImage(subtractedImage);
        List<DetectedInitializer> detectedInitializers = FindObjects(
            transformedImage,
            undistortedCroppedImage
        );
    ...
    {
        if (!isDetecting || activeObjects.Count == 0)
    ...
    List<DetectedInitializer> FindObjects(Mat image, Mat undistortedCroppedImage)
    {
        ...
        foreach (Point[] contour in contours.OrderByDescending(contour => Cv2.ContourArea(contour)))
        {
            ...
            foreach (InitializedObject initializedObject in initializedObjects)
            {
                ...
                double matchShapeScore = Cv2.MatchShapes(
                    contour,
                    initializedObject.Contour,
                    ShapeMatchModes.I1
                );
                ...
                if (
                    initializedObject.CheckColor
                    && !AreHuesSimilar(hue, initializedObject.ColorHue)
                    && !AreHuesSimilar(hue, initializedObject.WhiteHue)
                )
                    continue;

                Vector2 centroidInCanvasSpace = objectInitializer.CalculateAndConvertCentroid(
                    contour,
                    image,
                    fullImage.rectTransform
                );
                Point centroidPoint =
                    new((int)centroidInCanvasSpace.x, (int)centroidInCanvasSpace.y);
                float rotationAngle = objectInitializer.GetRotationAngle(contour);

                DetectedInitializer detectedObject =
                    new()
                    {
    ...
    public void ProcessDetectedObjects(List<DetectedInitializer> detectedInitializers, Mat image)
    {
        foreach (DetectedInitializer detectedInitializer in detectedInitializers)
        {
            ...
            {
                float rotationAngle = objectInitializer.GetRotationAngle(
                    detectedInitializer.contour
                );
                if (float.IsNaN(rotationAngle))
                    continue;

                Point[] normalizedContour = objectInitializer.NormalizeContour(
                    detectedInitializer.contour,
                    detectedInitializer.centroidInCanvasSpace,
                    image.Width,
                    image.Height,
                    rotationAngle
                );
                GameObject gameObject = objectInitializer.VisualizeObject(
                    normalizedContour,
                    detectedInitializer.centroidInCanvasSpace,
                    detectedInitializer.rotationAngle,
                    detectedInitializer.initializedObject.Color,
                    detectedInitializer.initializedObject.Name
                );
                gameObject.GetComponent<DetectedObject>().initializedObject =
                    detectedInitializer.initializedObject;
    ...
    {
        int positionHashX = Mathf.RoundToInt(
            detectedInitializer.centroidInCanvasSpace.x * (1f / positionMargin)
        );
        int positionHashY = Mathf.RoundToInt(
            detectedInitializer.centroidInCanvasSpace.y * (1f / positionMargin)
        );
        float sizeHash = Mathf.RoundToInt(
            detectedInitializer.initializedObject.Contour.Length * (1f / positionMargin)
        );
        return new DetectedValues
        {
            centroidInCanvasSpace = new Vector2(positionHashX, positionHashY),
            sizeHash = sizeHash
        };
    }
    ...
    private void UpdateGameObject(GameObject gameObject, DetectedInitializer detectedInitializer)
    {
        ...
        if (
            gameObject == null
            || float.IsNaN(detectedInitializer.centroidInCanvasSpace.x)
            || float.IsNaN(detectedInitializer.centroidInCanvasSpace.y)
        )
            return;

        gameObject
            .GetComponent<DetectedObject>()
            .UpdatePosition(detectedInitializer.centroidInCanvasSpace);
        ...
        foreach (Point point1 in contour1)
        {
            foreach (Point point2 in contour2)
            {
                float distance = Mathf.Sqrt(
                    Mathf.Pow(point1.X - point2.X, 2) + Mathf.Pow(point1.Y - point2.Y, 2)
                );
